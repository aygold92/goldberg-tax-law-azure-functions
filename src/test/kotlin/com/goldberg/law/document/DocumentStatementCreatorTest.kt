...
import com.goldberg.law.entity.EntityValues.TRANSACTION_ID
...
class DocumentStatementCreatorTest {
    ...
    fun `single statement - all fields mapped, transactions wired, verifier called`() {
        ...
        assertThat(result.single()).usingRecursiveComparison()
            .ignoringFields("statementDetails.statementId", "transactions.transactionId")
        ...
        verify(bankStatementVerifier).getSuspiciousReasons(
            argThat<StatementDetails> { copy(statementId = STMT_ID) == newStatementDetails() },
            argThat<List<TransactionDetails>> { map { t -> t.copy(transactionId = TRANSACTION_ID) } == transactionDetails },
            eq(newClassification())
        )
    }
    ...
    inner class MultipleStatementsPath {
        ...
        fun `groups transactions by beginning balance records and maps account summary per statement`() {
            ...
            verify(bankStatementVerifier).getSuspiciousReasons(
                argThat<StatementDetails> { copy(statementId = STMT_ID) == stmtDetails1 },
                argThat<List<TransactionDetails>> { map { t -> t.copy(transactionId = TRANSACTION_ID) } == transactionDetails1 },
                eq(classification)
            )
            verify(bankStatementVerifier).getSuspiciousReasons(
                argThat<StatementDetails> { copy(statementId = STMT_ID) == stmtDetails2 },
                argThat<List<TransactionDetails>> { map { t -> t.copy(transactionId = TRANSACTION_ID) } == transactionDetails2 },
                eq(classification)
            )
        }
    ...
    inner class IsBeginningBalanceRecord {
        ...
        @Test
        fun `non-null amount is not a beginning balance record even if description matches`() {
            with(DocumentStatementCreator.Companion) {
                assertThat(newTransactionDetails(description = "Beginning Balance", amount = BigDecimal("100.00")).isBeginningBalanceRecord()).isFalse()
            }
        }
