...
```json
{
  ...
  "banks": {
    ...
    "chase_checking":  { "name": "Chase",           "source": "discovered" }
  },
  "unassigned_pages": [],
  "warnings": []
}
...
- `banks`: one entry per distinct bank found this run. `source` is `memory` if the bank's patterns came from `/mnt/memory/bank-patterns/`, or `discovered` if derived this run.
