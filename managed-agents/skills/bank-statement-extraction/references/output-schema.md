...
- `isCreditCard` — **(R)** boolean: `true` for a credit-card statement, `false` for a deposit account (checking/savings/money market/etc.). Normally passed in from the splitting step and echoed back as-is; drives the reconciliation identity (see Reconciliation in SKILL.md) and constrains `account_type`. Only override it if the page itself unmistakably contradicts what you were given — in that case use what the page shows, and say so in `warnings`. A statement that genuinely mixes both is rare — handle each account on its own terms and note it in `warnings`.
...
- `check_number` — **(O)** when it's a check and a number is shown; else `null`. This field should be populated when the description clearly indicates that it is a check with a number, i.e. "Check 1001"
...
- **R** fields *should* exist. Banks do weird things, and clients do weird things (redactions, misprinted/misordered pages, etc.) so one can legitimately be missing — but that's unusual and should go in `warnings`.
- **O** fields are present on some types of bank statements and not others. Absent → `null`, no warning needed. (E.g. some banks show total credits/debits, some only counts, some neither.)
