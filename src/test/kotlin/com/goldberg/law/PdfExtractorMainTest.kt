...
class PdfExtractorMainTest {
    @Mock
    private val pdfLoader: PdfLoader = mock()
    @Mock
    private val splitter: PdfSplitter = mock()
    @Mock
    private val classifier: DocumentClassifier = mock()
    @Mock
    private val dataExtractor: DocumentDataExtractor = mock()
    @Mock
    private val csvWriter: CsvWriter = mock()
    @Mock
    ...
    val main = PdfExtractorMain(
        pdfLoader, splitter, classifier, dataExtractor, csvWriter, accountNormalizer, checkToStatementMatcher, accountSummaryCreator
    )
    ...
    fun testNormal() {
        ...
        verify(csvWriter).writeCheckSummaryToCsv(CHECK_SUMMARY_FILENAME, mapOf(
            CHECK_DATA_KEY_1000 to CHECK_DATA_1000,
            CHECK_DATA_KEY_1001 to CHECK_DATA_1001
        ), setOf(), setOf())
    }
    ...
    fun testChecksNotUsed() {
        ...
        verify(csvWriter).writeCheckSummaryToCsv(CHECK_SUMMARY_FILENAME, mapOf(
            CHECK_DATA_KEY_1000 to CHECK_DATA_1000,
            CHECK_DATA_KEY_1001 to CHECK_DATA_1001
        ), setOf(), setOf(CHECK_DATA_KEY_1001))
    }
    ...
    companion object {
        ...
        val CHECK_DATA_1000 = CheckData(ACCOUNT_NUMBER, 1000, "test", FIXED_TRANSACTION_DATE, -500.0, PAGE_METADATA_3_1)
        val PAGE_METADATA_3_2 = PageMetadata(BATES_STAMP + "32", "test3", 2)
        val CHECK_DATA_KEY_1001 = CheckDataKey(ACCOUNT_NUMBER, 1001)
        val CHECK_DATA_1001 = CheckData(ACCOUNT_NUMBER, 1001, "test", FIXED_TRANSACTION_DATE, -500.0, PAGE_METADATA_3_2)
