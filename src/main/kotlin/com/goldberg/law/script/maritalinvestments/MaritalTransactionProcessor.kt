...
abstract class MaritalTransactionProcessor(
    ...
    abstract fun getCurrentCashTotals(isPostMarriage: Boolean): Pair<BigDecimal, BigDecimal>
    /** subtract the cash from the balance and return the amount consumed of preMarital and shared holdings */
    abstract fun consumeCash(amount: BigDecimal, isPostMarriage: Boolean): BigDecimal
    abstract fun addCash(amount: BigDecimal, preMaritalPercent: BigDecimal, isPostMarriage: Boolean)
    ...
    fun processMaritalTransactions(): ProcessMaritalTransactionsOutput {
        ...
        sortedTransactions.forEachIndexed { idx, transaction ->
            try {
                ...
                if (transaction.getStatementDate() != currentStatementDate) {
                    val (preMaritalCashHoldings, sharedCashHoldings) = getCurrentCashTotals(isPostMarriage)
                ...
                val percent: BigDecimal? = when (transaction.type) {
                    TransactionType.BUY -> {
                        ...
                        val preMaritalCashPercent = consumeCash(transaction.amount, isPostMarriage)

                        val preMaritalQuantity = transaction.quantity!!.multiplyScale(preMaritalCashPercent)
                        preMaritalHoldings.add(transaction.symbol, preMaritalQuantity)
                        sharedHoldings.add(transaction.symbol, transaction.quantity!! - preMaritalQuantity)
                    ...
                    TransactionType.SELL -> {
                        ...
                        addCash(transaction.amount, preMaritalInstrumentPercent, isPostMarriage)
                    ...
                    TransactionType.RE_INVESTMENT -> {
                        ...
                        val (preMaritalQuantity, sharedQuantity) = if (transaction.symbol == cashSym) getCurrentCashTotals(isPostMarriage)
                        else Pair(
                            preMaritalHoldings.getOrDefault(transaction.symbol, BigDecimal.ZERO),
                            sharedHoldings.getOrDefault(transaction.symbol, BigDecimal.ZERO)
                        )
                        ...
                        if (transaction.symbol == cashSym) {
                            addCash(totalQuantityOrAmount, preMaritalInstrumentPercent, isPostMarriage)
                        } else {
                    ...
                    TransactionType.DEPOSIT -> {
                        ...
                        if (transaction.classificationOverride == TransactionClassification.Marital ||
                            ...
                            addCash(transaction.amount, BigDecimal.ZERO, isPostMarriage)
                            BigDecimal.ZERO
                        } else {
                            addCash(transaction.amount, BigDecimal.ONE, isPostMarriage)
                            BigDecimal.ONE
                        }
                    }
                    ...
                    TransactionType.WITHDRAWAL -> {
                        ...
                        consumeCash(transaction.amount, isPostMarriage)
                    }
        ...
        val (preMaritalCashHoldings, sharedCashHoldings) = getCurrentCashTotals(currentStatementDate.after(marriageDate))
        holdingsReport[currentStatementDate.toTransactionDate()] = HoldingsReport(
            preMaritalHoldings.plus(cashSym to preMaritalCashHoldings).toMap(),
            sharedHoldings.plus(cashSym to sharedCashHoldings).toMap()
        )
