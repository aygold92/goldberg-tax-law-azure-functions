...
class DeleteInputDocumentFunction(
    ...
    fun run(
        ...
        if (metadata.split) {
            (1..metadata.totalpages!!).toList().mapAsync {
                dataManager.deleteSplitFile(req.clientName, PdfPageData(req.filename, it))
            }
        }
        ...
        if (metadata.analyzed) {
            (1..metadata.totalpages!!).toList().mapAsync {
                dataManager.deleteModelFile(req.clientName, PdfPageData(req.filename, it))
            }
        }
